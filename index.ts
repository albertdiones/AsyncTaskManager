...
export class PaddedScheduleManager implements AsyncTaskManagerInterface {
  ...
  _addToQueue(task: () => Promise<any>, timeout: number): Promise<any> {
    
  }

  _queueAndExecute(task: () => Promise<any>, timeout: number): Promise<any> {
    if (timeout > 0) {// negative delay will also be done instantly)  
        this.queue.push(
            () => Bun.sleep(timeout)
        );
  ...
  add(task: () => any, name?: string): Promise<any> {
      ...
      return this._addToQueue(task,timeout);
    }
}